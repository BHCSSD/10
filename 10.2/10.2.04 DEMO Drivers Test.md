...
```javascript
// Model
let logo    //go find an emoji for a logo.
// Remember emojis are strings, put them in "
let testScore = 0
//Step 2   make a variable to store users age
...
function draw() {
  ...
  //Step 1.  Show your emoji logo... push()  pop() to help with formatting
  ...
  // step 4 Logic for if statements to display eligibility message
    // if(){
...
function keyPressed() {
    ...
    // step 3.  ask the users their age
