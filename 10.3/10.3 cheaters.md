...
#### 1. The Eye: `get(mouseX, mouseY)`

The `get()` function acts like a digital magnifying glass. It looks at the exact spot where your cursor is and "grabs" the color.

When the computer grabs a color, it doesn't just say "Red" or "Blue." It creates a **list of 4 numbers** that looks like this:

`[Red, Green, Blue, Transparency]`

  * If you hover over the **Red** square, `get()` grabs these numbers: **`[255, 0, 0, 255]`**
  * The first number is the **Red** value. `255` means it is "fully on."


#### 2. The Brain: `if(col[0] == 255)`
* **`col[0]`**: look at the **first number** in your color list (the Red value).
    * *Note: In coding, we start counting at 0*
...
function draw() {
  ...
  let col = get(mouseX, mouseY); // gets the mouse colour value
  text(col, 20, 40)

  if (col[0] == 255) {  // col[0] is the first value
    text("your colour is red", 20, 350);
  } else if (col[1] == 128) {   // col[1] is the second value
    text("your colour is green", 20, 350);
  }//end of if
}//end of draw
...
```
### Count Down Timer
This code uses `millis()` to check if a certain amount of time has passed. When 1 second is up, it prints `"waited"` to the console. The program keeps running smoothly because it does **not** use `delay()`.


```javascript

let time = 0 // Keeps track of the last time the wait finished

function draw() {

  // Runs once every 1000 ms (1 second)
  if (wait(1000)) {   // use wait(1000)  in an if to make this work. 
    print("waited")
    print("do something in your code")
  }
...
/*
NOTE:
This wait() function uses one shared timer.
Only use one wait at a time, or order them
from smallest to largest wait time.
*/
```
